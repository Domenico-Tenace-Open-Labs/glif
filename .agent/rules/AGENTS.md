...
- Download generated QR codes as PNG files (web) or save directly to the device gallery (native Android/iOS via Capacitor)
- Work offline as a Progressive Web App (PWA)
- Run as a native application on mobile thanks to Capacitor
...
- **Capacitor** - Cross-platform API and runtime layer to build native apps with web tech
...
- **Capacitor Plugins** - Includes core, filesystem, and community media to perform native save operations on Android/iOS
...
npx cap sync         # Sync web build to the native app shell
npx cap open android # Launch the native project in Android Studio
```
...
- **Web**: Creates temporary download link and uses timestamp for uniqueness
- **Native App**: Interfaces with `@capacitor/filesystem` to cache the file and `@capacitor-community/media` to securely store it directly in the device's main gallery along with creating dedicated albums
